...
## 重定向
"重定向"这个词的含义是改变方向或改变路径。在计算机科学中，当我们谈论输入/输出重定向时，我们实际上是在改变数据流的路径。

默认情况下，Unix 和 Linux 命令的输入来自键盘（也就是标准输入，或 stdin），输出则显示在屏幕上（也就是标准输出，或 stdout）。错误信息也会显示在屏幕上（这被称为标准错误，或 stderr）。

当我们使用重定向操作符（如 `>`, `>>` 或 `<`）时，我们实际上是在改变这些默认的数据流路径。例如，我们可以将输出重定向到一个文件，或者从一个文件中获取输入。这就是为什么这个操作被称为"重定向"。

总的来说，"重定向"这个词描述的是这个操作的本质：改变数据流的方向。

命令重定向（Command Redirection）是 Linux 和其他 Unix-like 系统的一个功能，它允许你改变命令的输入/输出源。默认情况下，命令的输入来自键盘，输出则显示在屏幕上。但是，你可以使用重定向操作符来改变这种行为。

在 Linux 中，常见的重定向操作符有：

1. `>`：将命令的输出重定向到一个文件中。如果文件已存在，它会被覆盖。

   例如，`ls > file.txt` 会将 `ls` 命令的输出保存到 `file.txt` 文件中。

2. `>>`：将命令的输出追加到一个文件的末尾。如果文件不存在，它会被创建。

   例如，`ls >> file.txt` 会将 `ls` 命令的输出追加到 `file.txt` 文件的末尾。

3. `<`：将命令的输入从一个文件中读取。

   例如，`sort < file.txt` 会将 `file.txt` 文件的内容作为 `sort` 命令的输入。

这就是命令重定向的基本概念。通过使用重定向，你可以更灵活地控制命令的输入和输出。
